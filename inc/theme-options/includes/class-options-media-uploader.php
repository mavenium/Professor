...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('پرونده ای انتخاب نشده') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'بارگذاری' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'حذف' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'لطفا وردپرس خود را بروزرسانی کنید.' ) . '</i></p>';
		}
		...
		if ( $value != '' ) {
			$remove = '<a class="remove-image">حذف</a>';
